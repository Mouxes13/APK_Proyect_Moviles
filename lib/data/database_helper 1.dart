...
class InfoVal {
  ...
  Future<List<Aspi>> obtenerAspirantes() async {
    var dbClient = await db;
    List<Aspi> aspirantesList = List();
    ...
    if (queryList != null && queryList.length > 0) {
      for (int i = 0; i < queryList.length; i++) {
        aspirantesList.add(Aspi(
          queryList[i]['id'],
          queryList[i]['nombre'],
          queryList[i]['correo'],
          queryList[i]['prepa'],
          queryList[i]['municipio'],
        queryList[i]['movil'],
        queryList[i]['email']
        ));
      }
...
Future<Aspi> getEmpleadoAPI( Aspi aspirante
) async{

  var url ="http://sistemas.upiiz.ipn.mx/isc/sira/api/actionAddAspiranteApp.php?id=1&nombre=${aspirante.name}&email=${aspirante.email}&movil=${aspirante.movil}&accion=agregar";
  ...
  if(respuestaAPI.statusCode==200){
    var miEmpleadoAPIJSON = json.decode(respuestaAPI.body);
    print(miEmpleadoAPIJSON);
  }
